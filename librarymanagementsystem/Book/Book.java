...
public class Book {
    ...
    int nearestBookID;
    int bookRenewalCount;
    int booksInShelfCount = 0;
    Date returnedDate;
    ...
    public Book addBook(int bookNo) {
        bookRenewalCount = 0;
        this.bookNo = bookNo;
        bookName = getInputString("Enter Book Name: ");
        author = getInputString("Enter Author: ");
        publishedIn = getInputString("Enter Published Year: ");
        while (!Utils.isAValidYear(publishedIn)) {
            publishedIn = Utils.getValidYear();
        }
        ...
        for (int i = 0; i < totalCopies; i++) {
            book = new Book();
            book.copyContents(bookNo, bookName, author, publishedIn, bookPrice, genre, 0);
            bookCopies.add(book);
        }
    ...
    public void copyContents(int bookNo, String bookName, String author, String publishedIn, double bookPrice, Genre genre, int bookRenewalCount) {
        this.bookNo = bookNo;
        ...
        isAvailable = true;
        this.bookRenewalCount = bookRenewalCount;
    }
                ...
                if ((bookCopy.bookID == bookID) && (!(bookCopy.bookLocation.contains("G")))) {
                    ...
                    if (patron.booksOnHold.size() < 2) {
                        if (bookCopy.isAvailable == true) {
                            ...
                            book.availableCopies--;
                            book.borrowedCopies++;
                            bookCopy.bookLocation = Constants.REQUESTED;
                        ...
                        System.out.println("Book Request Limit Reached!!");
                        Book returnBook = new Book();
                        returnBook.bookID = -1;
                        return returnBook;
                    }
    ...
    public static void displayForAddBookCopies(int bookNo) {
        ...
        while (!Utils.checkPositiveIntValue(totalCopiesToBeAdded)) {
            totalCopiesToBeAdded = Utils.getValidPositiveIntValue();
        }
        int bookFound = 0;
        for (Book book : Resources.books) {
            if (book.bookNo == bookNo) {
                for (int i = 0; i < totalCopiesToBeAdded; i++) {
                    ...
                    addBook.copyContents(book.bookNo, book.bookName, book.author, book.publishedIn, book.bookPrice, book.genre, 0);
                    bookCopied++;
        ...
        for (Book book : Resources.books) {
            for (Book bookCopy : book.bookCopies) {
                if (bookCopy.bookID == bookID) {
                    ...
                    bookCopy.toBeReturnedDate = Utils.parseDate("30-12-3000");
                    bookCopy.bookRenewalCount = 0;
                }
            }
        }
    }
